...
2. **Questions**, each with the default in brackets and the reason in one line. Database: `embedded` PGlite in `./data/pg` (default, nothing to install) · `docker` (Compose Postgres 18, real database) · `url` (own `DATABASE_URL`, typed masked). Port `[3000]`. Tiles: `watercolor` via Stadia (default, keyless on localhost) · `osm` (never needs a key, and the way to open the app from a phone on the same network). Allowed origins `[http://localhost:5173,http://localhost:3000]`.
