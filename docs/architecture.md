...
- `ChatClient` wraps the global `WebSocket` (present in browsers and React Native): reconnect with jittered backoff, Zod-parsed inbound events written into the store, backfill through the typed HTTP client on (re)connect.
