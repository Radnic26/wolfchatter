...
scripts/         review.sh (headless self-review)
...
- **Gates:** Biome (`biome ci`), `tsc --noEmit` per workspace, tests with coverage, `vite build`, `npm audit`, Docker build. The same checks run locally through versioned git hooks (`.githooks/`: Biome on staged files and typecheck before commit, a Conventional Commits check on the message, tests before push).
- **Security baseline:** validate at every boundary, explicit CORS/Origin allowlists, secure headers on `/api/*`, no secrets in the repo (none are needed; `.env.example` documents defaults), env validated at boot, no PII stored or logged, React escaping only.
...
- **Review:** an independent reviewer agent in a fresh context (read-only tools, explicit "report gaps, not style" criteria, file:line and a reproduction for every finding) plus the built-in `/code-review`; findings triaged as fixed / not fixed with reason / needs human; raw output and curated report committed. Any new dependency not in §1 is a review finding.
