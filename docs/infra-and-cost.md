...
| One cold page load, gzipped: JS 137 KB, CSS 10 KB, HTML 0.6 KB, two woff2 fonts 37 KB | ≈ 184 KB; the audit re-measured this after round 10 changed `vite.config.ts` and found the bundle unchanged, the fonts 3 KB smaller than the rounding here had them |
...
| App service | RAM $10 per GB-month, CPU $20 per vCPU-month, both on use | 0.48 at the measured 49 MiB idle; 1.13 at the 116 MiB the audit measured under NFR-3's assumed load; CPU ≈ 0 either way | Memory tracks open sockets at about 42 KiB each, so the figure moves with concurrency rather than with traffic; the credit absorbs an average of up to 0.5 GB, which is roughly 8,000 sockets |
...
**Before the first deploy.** The deployed hostname is registered in the Stadia dashboard, or the map answers 401 everywhere but on localhost (`architecture.md` §10), and `ALLOWED_ORIGINS` names the deployed origin, because an upgrade from anywhere else is refused (§5). Registering the hostname is necessary and was not sufficient: the audit found that the server's own `Referrer-Policy` stripped the `Referer` that registration is checked against, so the tiles answered 401 wherever they were served from (finding A1-1, repaired). The three findings this document declared open have since been closed by the review and audit rounds. Every platform priced here terminates TLS at its own proxy, so the process sees one address for everyone unless `TRUSTED_CLIENT_HEADER` names the header that proxy appends — set it, or the per-IP allowance of 40 writes refilled at 2 a second becomes the whole site's and legitimate posts get `429` at the 5-a-second peak.
...
NFR-3 fixes the ceiling this design is tested to: 500 open sockets and 50 messages a second with a p95 broadcast latency under 250 ms. **The audit has now measured past it** ([docs/audit/README.md](audit/README.md), 2026-09-06), so the provisional halves this section used to carry are replaced by what one process actually did. Four numbers are watched, produced by the audit's load script and by the platform's metrics: concurrent sockets, p95 broadcast latency, the process's CPU (Node uses one core, so 1 vCPU sustained is the ceiling) and the p95 of the two list queries, which NFR-1 holds under 50 ms.

**What one process was measured doing.** At NFR-3's 500 sockets and 50 messages a second, p95 from `POST` to delivery was 4 ms against a 250 ms budget, with no errors and no dropped sockets; ten minutes of the same held 6 ms with memory flat. Pushed further, it served **8,000 concurrent sockets** and, separately, **1,860 messages a second**, both under 70% of one core and both still under 5 ms. No breaking point was reached: the load generator ran out first, so those two numbers are a floor rather than a ceiling.

**Memory is the constraint, not CPU**, and it is the number that sets the triggers below. It tracks connected sockets at roughly 42 KiB each over a 50 MiB idle baseline — 116 MiB at 500 sockets, 389 MiB at 8,000 — and is released when they close. On the Hobby plan the $5 credit absorbs an average of about 0.5 GB, which is reached at roughly 8,000 sockets, so the bill moves before the process does.

**Step 1: a second instance.** Fires when the weekly peak passes 3,000 concurrent sockets — comfortably below both the measured floor and the memory the credit covers — when p95 broadcast latency at that peak passes 125 ms, when the service sits at 0.5 vCPU sustained, or earlier, when a deploy is no longer allowed to drop every open socket. What changes: a `Broadcaster` over Postgres `LISTEN/NOTIFY`, exactly as `architecture.md` §5 describes it — the write route publishes the row's id, every instance re-reads the row and fans it out to its own sockets, ordering stays the database's — plus one direct connection per instance for `LISTEN`. Not a line of the routes changes, and a lost notification is already covered, because the row is in the database before it is announced and the client back-fills by id. On Railway this is a second replica of the same service (Hobby allows six; routing is random, which raw WebSockets tolerate).
